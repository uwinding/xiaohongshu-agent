...
**职责**：抓取小红书穿搭赛道热搜话题和风格趋势，为选品和穿搭提供灵感方向。

| 维度 | 描述 |
|------|------|
| **输入** | 时间范围（日/周）、品类（女装/大码/小个子）、风格标签 |
| **处理逻辑** | 爬取小红书搜索页热门笔记 → 提取标题关键词 → LLM 汇总趋势摘要 → 输出热门元素（如"今夏碎花""新中式半裙"） |
| **输出** | `{ keywords: [], style_trends: [], hot_items: [], trend_summary: string }` |
| **LLM Prompt 要点** | 要求输出结构化 JSON，包含热度评分（1-10）、适用体型标签 |
| **PM 交付物** | TrendRadar System Prompt、输出规范定义、趋势分析示例 |
...
**职责**：根据穿搭方案和商品信息，生成小红书风格图文文案。

| 维度 | 描述 |
|------|------|
| **输入** | 穿搭描述 `outfit_desc`、商品列表、博主人设 |
| **处理逻辑** | LLM 生成小红书爆款风格的标题 → 正文（穿搭心得+单品亮点+购买理由） → 话题标签 → 商品标记信息 |
| **输出** | `{ title: string, content: string, hashtags: [], product_tags: [{name, url}] }` |
| **文案风格** | 口语化、亲切感、emoji点缀、包含"姐妹们""绝绝子""冲"等小红书高频词；大码强调"显瘦""自信"，小个子强调"显高""比例" |
